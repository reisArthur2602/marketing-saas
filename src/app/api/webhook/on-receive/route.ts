...
export const POST = async (request: NextRequest) => {
  try {
    ...
    // 🔹 Se houver campanhas, processa palavras-chave
    if (campaigns.length > 0) {
      for (const campaign of campaigns) {
        const keywordFound = campaign.keywords.find((k) =>
          text.message.toLowerCase().includes(k.word.toLowerCase()),
        );
        if (!keywordFound) continue; // palavra-chave não bateu, tenta próxima campanha
    ...
    const fallbackMessage =
      session.user.defaultFallbackMessage || genericMessage;
